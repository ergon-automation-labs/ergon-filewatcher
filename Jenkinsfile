pipeline {
  ...
  environment {
    BOT_NAME = "filewatcher_bot"
    RELEASE_DIR = "/opt/ergon/releases/${BOT_NAME}"
    GITHUB_REPO = "ergon-automation-labs/ergon-filewatcher"
  }
    ...
    stage('Run Migrations') {
      steps {
        sh '''
          ...
          RELEASE_BIN="${RELEASE_DIR}/current/filewatcher_bot/bin/filewatcher_bot"
          ...
          echo "Running: $RELEASE_BIN eval 'BotArmyFileWatcher.Release.create()'"
          $RELEASE_BIN eval 'BotArmyFileWatcher.Release.create()'

          echo "Running: $RELEASE_BIN eval 'BotArmyFileWatcher.Release.migrate()'"
          $RELEASE_BIN eval 'BotArmyFileWatcher.Release.migrate()'
  ...
  post {
    success {
      sh '''
        ...
        if [ -f ./release-artifact/bot_army_filewatcher/releases/start_erl.data ]; then
          VERSION=$(awk '{print $2}' ./release-artifact/bot_army_filewatcher/releases/start_erl.data)
        fi
